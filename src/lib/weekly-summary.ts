...
export function generateWeeklySummaryText(data: WeeklySummaryData): string {
  ...
  const lines: string[] = [];
  lines.push(`📅 Weekly Training Summary - ${weekRange}`);
  lines.push('');
  lines.push(`✅ Completed: ${data.completedSessions} / ${data.plannedSessions} sessions`);
  lines.push(`📊 Total Load: ${data.totalLoad} TSS`);
  lines.push('');
  ...
  lines.push('Generated by Athlete Space');
