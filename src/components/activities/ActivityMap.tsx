...
export function ActivityMap({ coordinates }: ActivityMapProps) {
  // Debug logging
  useEffect(() => {
    console.debug("[ActivityMap] Received coordinates:", coordinates?.length || 0, "points");
    if (coordinates && coordinates.length > 0) {
      console.debug("[ActivityMap] Sample coordinate:", coordinates[0]);
    }
  }, [coordinates]);

  // Guard: Ensure coordinates is a valid array with data before rendering
  ...
  // At this point, coordinates is guaranteed to be a non-empty array
